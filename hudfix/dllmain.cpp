...
bool replaceMemory(void* dest, void* source, size_t len, bool rdata = false) {
    DWORD curProtection;
    DWORD newProtection = rdata ? PAGE_READWRITE : PAGE_EXECUTE_READWRITE;
    if (!VirtualProtect(dest, len, newProtection, &curProtection)) {
        log("ERROR: VirtualProtect failed to override protection, error code: %i. aborting...", GetLastError());
        return false;
    }
...
void onHudLoaded() {
    ...
    for (std::set<std::string>::iterator it = g_nodesLeft.begin(); it != g_nodesLeft.end(); ++it) {
        if ((tmp = meme::findNode((*it).c_str())) != nullptr)
            tmp->setNodePos(tmp->getNodePosX() - g_hudOffset, tmp->getNodePosY());
        else
    ...
    for (std::set<std::string>::iterator it = g_nodesFillRight.begin(); it != g_nodesFillRight.end(); ++it) {
        if ((tmp = meme::findNode((*it).c_str())) != nullptr)
            tmp->setNodeSize(g_hudOffset, tmp->getNodeSizeY());
        else
            log("ERROR: %s Not Found!", (*it).c_str());
    }
...
BOOL WINAPI DllMain(HINSTANCE hModule, DWORD dwReason, LPVOID lpReserved) {
    ...
    case DLL_PROCESS_ATTACH:
        ...
        float aspect_ratio = 1.3333333f; // default 4:3
        bool debug = false;
        bool explicitRatio = false;
        ...
        if (config.is_open())
        {
            ...
            while (getline(config, line))
            {
                ...
                if (splited.size() != 0) {
                    ...
                    else if (splited.at(0) == "aspect_ratio" && splited.size() == 2) {
                        explicitRatio = true;
                        aspect_ratio = std::stof(splited.at(1));
                    }
        ...
        if (debug) {
            ...
            log("\n---------------------\nBF2 stretched HUD FIX v0.3\n---------------------");
        }

        if (explicitRatio) {
            log("skipping D3D hook... fix will be applied for aspect_ratio=%f", aspect_ratio);
            update_offsets(aspect_ratio);
        }
        else {
